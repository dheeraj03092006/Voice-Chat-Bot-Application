...
class LiveTranscriber:
    ...
    async def listen(self) -> str:
        ...
        options = LiveOptions(
            ...
            channels=1,
            sample_rate=16000,
            endpointing=100,  # Milliseconds of silence to determine end of speech (faster)
            smart_format=True,
        )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--api":
        # API mode: return both user_text and ai_response as JSON
        async def api_mode():
            config = Config()
            assistant = VoiceAssistant(config)
            print("Listening...", flush=True)
            user_text = await assistant.transcriber.listen()
            ai_response = assistant.llm_processor.generate_response(user_text)
            assistant.synthesizer.speak(ai_response)
            print(json.dumps({"user_text": user_text, "ai_response": ai_response}))
